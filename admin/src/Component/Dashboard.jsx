    ...
    useEffect(() => {
        const fetchAppointment = async () => {
            try {
                ...
                setAppointments({});
                console.log("some error occure during fetch the appointment", error)
                toast.error(error.response.data.message);
            }
        }
        fetchAppointment();
    }, []);
    const handelUpdateStatus=async(appointmentId,status)=>{
        try {
            ...
            <section className="dashboard page">
                <div className="banner">
                    <div className="firstBox">
                        <img src="/doc.png" alt="docImg"></img>
                        <div className="content">
                            ...
                            <p>Hii {`${admin.firstName} ${admin.lastName}`} this is your dashboard please handel the panding appointments and messages</p>
                        </div>
                    </div>
                            ...
                            {appointments && appointments.length > 0 ? appointments.map(appointment => {
                                return (
                                    <tr key={appointment._id}>
                                        ...
                                        <td>
                                            <select className={appointment.status === "Pending" ? "value-pending" : appointment.status === "Rejected" ? "value-rejected" : "value-accepted"} value={appointment.status} onChange={(e) => handelUpdateStatus(appointment._id,e.target.value)}>
